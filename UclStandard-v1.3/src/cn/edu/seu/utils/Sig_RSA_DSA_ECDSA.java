...
public class Sig_RSA_DSA_ECDSA {  
    public static void main(String[] args) {  
      Sig_RSA_DSA_ECDSA.get_sig(1);//进行数字签名类型选择，看最新版本的ucl   1是RSA 2是ECDSA 3是DSA   暂时后面两个（ECC和HMAC 没有找到方法）
    }  
    public static void get_sig(int style){//根据传入的类型进行操作
    	 try {  
    ...
    public static void run(ArrayList<String> strings ) {  
        // 数字签名生成密钥  
        ...
        try {  
            ...
            in.close();  
            String myinfo = "我是来自东南大学的张晓刚，很高兴能够在未来网络研究室工作。"; // 要签名的信息  
            // 用私钥对信息生成数字签名  
